...
\begin{frame}{Computing Environment for Astrophysics}
  ...
  \item Calculations often done on specialised `high-performance computing' (HPC) machines having large filesystems and large RAM; calculations are often broken into pieces that can be performed simulataneously (`in parallel') across many processors.
  \item Much useful software made freely available within the community. Software quality is usually high; documentation quality is more variable.
  \end{itemize}
\end{frame}

\begin{frame}{Local Computing Environment}
  \begin{itemize}
  \item You will probably have your own local machine:
    \begin{itemize}
    \item PC (Windows)
    \item Mac
    \item Linux
    \end{itemize}
  \item In addition there are shared HPC machines:
    \begin{itemize}
    \item splinter
    \item Others? Legion?
    \end{itemize}
