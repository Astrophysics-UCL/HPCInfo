...
\begin{frame}{Python: Compiled}
  ...
  \item {Then execute it by calling \alert{\texttt{python~my\_program.py}}}
  
  \item {Even better:
  \begin{itemize}
   \item Put \alert{\texttt{\#!/usr/bin/env~python}} as the first line in my\_program.py
   \item Then run \alert{\texttt{chmod +x~my\_program.py}}
  \end{itemize}
  Then you can execute the program via \alert{\texttt{./my\_program.py}}}
  \end{itemize}
\end{frame}
...
\begin{frame}{Python}
  ...
  \begin{block}{Functions} 
       \texttt{def function\_name(arg1, arg\_vol=1, *args, **kwargs):} \\
       ...
       \texttt{function\_name(''HelloWorld'', *[1, 2, 3], **\{''foo'':''bar''\})}
  \end{block}
  \begin{block}{Classes}
       Can create objects with functions, but must initialise the arguments \\
       \texttt{class class\_name:} \\
       \texttt{def \_\_init\_\_(self, arg):} \\
       \texttt{self.argument = arg}
  \end{block}
\end{frame}
...
\begin{frame}{Scipy/Astropy/PyFITS/SymPy}
  ...
  \begin{block}{World Coordinate Systems}
      Can find the RA and Dec of pixels using \texttt{astropy.wcs.WCS} and convert RA and Dec to pixels using \texttt{astropy.wcs.wcs\_sky2pix()}
  \end{block}
...
\begin{frame}{Matplotlib Features}
  \fontsize{11pt}{11}\selectfont
  ...
  \begin{block}{Multiple Subplots}
     ...
     \texttt{plt.subplots\_adjust(hspace=0.5)} \\
     \texttt{ax1 = plt.subplot(121)} \\
     \texttt{ax2 = plt.subplot(122, sharex=ax1, sharey=ax1)}
  \end{block}
\end{frame}
