...
\node at (0,3) {or};
\draw [->,thick] (LM1.south) -- (Z.north) node [midway,left] {PuTTY};
\draw [->,thick] (Z.south) -- (S1.north) node [midway,left] {ssh};
\draw [->,thick] (LM2.south) -- (S2.north) node [midway,right] {PuTTY};
\end{tikzpicture}
\end{center}
\end{figure}
\end{frame}
...
\begin{frame}{Common and Useful Programs}
  ...
  \begin{block}{DS9}
    Introduce and show various features including aligning, panning, colour sets, scaling (also using right click), blinking, regions and annotating (producing a finder chart?)
  \end{block}

  \begin{block}{IRAF}
    Introduce modularity and how to get help (?) and edit parameter files (maybe do a quick aperture photometry demonstration?)
  \end{block}
\end{frame}
